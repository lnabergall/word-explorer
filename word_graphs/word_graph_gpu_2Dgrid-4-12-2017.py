...
NUM_NEIGHBOR_LIMIT = 4000
SMALL_ARRAY_LENGTH = 16
MAX_ARRAY_LENGTH = 4000
...
def find_adjacent_vertices(word_list, size_limit, ascending_order=False):
    ...
    threads_perblock = (4, pattern_instance_count)
    blocks_perdim = (
        (len(words) + (64 - 1)) // 64, 1)
...
def compute_neighbors(word_array, neighborhoods, size_limit, 
    ...
    """
    thread_num1, thread_num2 = cuda.grid(2)
    for i in range(word_array.size):
        if i == thread_num1:
            ...
            for j in range(neighbors_array.size):
                if neighbors_array[j] != 0:
                    neighborhoods[i, j + thread_num2*neighbors_array.size] = neighbors_array[j]
